...
import { useDeleteReturnedMutation, useGetReturnedsQuery, useGetTicketTypesQuery } from '@/services'
import { IReturned } from '@/services/canceled/types'
...
const DepartmentReturned: NextPage = () => {
  // ? Assets
  const { query, push } = useRouter()
  const userType = Number(query?.userType)
  // States
  const [stateData, setStateData] = useState<IReturned>()
  ...
  // ? Queries
  const { data: returnedsData, refetch } = useGetReturnedsQuery({ pageSize: 99999 })
  ...
  const onConfirmDelete = () => {
    deleteReturned({ id: deleteInfo.id })
  }
  const handleEdit = (data: IReturned) => {
    setStateData(data)
  ...
  return (
    <ProtectedRouteWrapper>
      ...
      {(isSuccessDelete || isErrorDelete) && (
        ...
        />{' '}
        <ReturnedModal
          title={`ویرایش دپارتمان مرجوعی`}
          mode="edit"
          userType={userType}
          returnedData={stateData}
          refetch={refetch}
...
export default dynamic(() => Promise.resolve(DepartmentReturned), { ssr: false })
