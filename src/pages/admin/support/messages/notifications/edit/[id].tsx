...
import { INotificationForm, ISmsMessageForm } from '@/types'
import {
...
const EditNotification: NextPage<Props> = () => {
  // ? Assets
  ...
  const {
    ...
    isLoading: isLoadingGetSelectedNotification,
  ...
  useEffect(() => {
    const loadData = () => {
      if (selectedNotification?.data) {
        ...
        console.log(selectedNotification.data, 'selectedNotification.data')
  ...
  // ? Create
  const [updateNotification, { isSuccess, isLoading, data, isError, error }] = useUpdateNotificationMutation()
  // ? Handlers
  const onSuccess = () => {
    push(`/admin/support/messages/notifications`)
  }
  ...
  const createHandler: SubmitHandler<INotificationForm> = (data) => {
    console.log(data, 'data fprm')

    const formData: INotificationForm = {
      id: selectedNotification?.data?.id,
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex flex-1">
                    <div className="bg-white w-full rounded-md shadow-item">
                      <h3 className="border-b p-6 text-gray-600 flex gap-2">ویرایش اعلان</h3>
...
export default dynamic(() => Promise.resolve(EditNotification), { ssr: false })
