...
const ProductObjectValueSelector: React.FC<Props> = (props) => {
  ...
  const handleClicked = (val: { id: string; name: string }, featureId: string, featureTitle: string) => {
  ...
  return (
    <div className="flex gap-2.5">
      {objectValues.map((val) => (
        <div className={`${
          selectedValue === val.id ? 'border-b-2 pb-1 border-red-600' : ''
        }`}>
          <button
            type="button"
            onClick={() =>
              feature?.id && feature?.title && handleClicked({ id: val.id, name: val.title }, feature.id, feature.title)
            }
            key={val.id}
