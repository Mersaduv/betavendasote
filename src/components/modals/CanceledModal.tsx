...
import { ICanceledForm, ICategory, ICategoryForm, IRegisterForm, IReturnedForm, ITicketTypeForm } from '@/types'
...
import { categorySchema, normalSchema, singleSchema, ticketTypeSchema } from '@/utils'
...
const CanceledModal: React.FC<Props> = (props) => {
  const { isShow, onClose, refetch, title, canceledData, mode } = props
  ...
  const {
    ...
    setValue,
    reset,
  } = useForm<IReturnedForm>({
    resolver: yupResolver(normalSchema),
    defaultValues: {
      isActive: true,
    },
  })
  ...
  useEffect(() => {
    if (canceledData) {
      setIsActive(canceledData.isActive)
      reset({
        id: canceledData.id,
        title: canceledData.title,
        isActive: canceledData.isActive,
      })
    }
  }, [canceledData])
  useEffect(() => {
  ...
  const onConfirm: SubmitHandler<ICanceledForm> = (data) => {
        ...
        >
          <Modal.Header notBar onClose={onClose}>
            <div className="text-start text-base flex gap-2">
              {' '}
              {title}
              <div className="text-sky-500">{canceledData?.title}</div>
            </div>
          </Modal.Header>
          <Modal.Body>
