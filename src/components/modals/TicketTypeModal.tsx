...
import { ICategory, ICategoryForm, ITicketTypeForm } from '@/types'
...
import { categorySchema, singleSchema, ticketTypeSchema } from '@/utils'
...
const TicketTypeModal: React.FC<Props> = (props) => {
  const { isShow, onClose, refetch, title, ticketTypeData, mode,userType } = props
  ...
  useEffect(() => {
    if (ticketTypeData) {
      setIsActive(ticketTypeData.isActive)
      reset({
        id: ticketTypeData.id,
        name: ticketTypeData.name,
        description: ticketTypeData.description,
        isActive: ticketTypeData.isActive,
        userTypes: ticketTypeData.userTypes,
      })
    }
  }, [ticketTypeData])
  useEffect(() => {
  ...
  const onConfirm: SubmitHandler<ITicketTypeForm> = (data) => {
        ...
        >
          <Modal.Header notBar onClose={onClose}>
            <div className="text-start text-base flex gap-2">
              {' '}
              {title}
              <div className="text-sky-500">{ticketTypeData?.name}</div>
            </div>
          </Modal.Header>
          <Modal.Body>
