...
const CategoryDetailModal: React.FC<Props> = (props) => {
  ...
  return (
    <>
      <div onClick={searchModalHanlders.open} className="text-cyan-500 cursor-pointer hover:text-cyan-300 py-3">
        جزئیات
      </div>
      <Modal isShow={isShowSearchModal} onClose={searchModalHanlders.close} effect="bottom-to-top">
        ...
        >
          <Modal.Header onClose={searchModalHanlders.close}>جزئیات دسته بندی</Modal.Header>
          <Modal.Body>
              ...
              <div className="overflow-y-auto lg:max-h-[500px]">
                <table className="w-[700px] md:w-full mx-auto">
                  <thead className="bg-sky-300">
                    <tr>
                      <th className="text-sm py-3 px-2 text-gray-600 font-normal text-center w-1/2">ردیف</th>
                      <th className="text-sm py-3 px-2 text-gray-600 font-normal text-center w-1/2">نام</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredCategories.length > 0 ? (
                      filteredCategories.map((item, index) => (
                        <tr key={item.id} className={`h-16 border-b ${index % 2 === 0 ? 'bg-gray-50' : ''}`}>
                          <td className="text-sm text-center farsi-digits">{index + 1}</td>
                          <td className="text-sm text-center farsi-digits">{item.name}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
